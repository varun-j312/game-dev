...
function startGame() {
  myGameArea.start();
  myGamePiece = new component(32, 32, "red", 0, 0);
  myCamera = new camera();
}
...
function updateGameArea() {
  ...
  //camera movement
  if( (myGamePiece.x > ((myGameArea.canvas.width / 2) - myGamePiece.width/2) && (myGamePiece.speedX!=0) && myGamePiece.x < ((myGameArea.canvas.width / 2) + myGamePiece.width/2) ||
       myGamePiece.y > ((myGameArea.canvas.height / 2) - myGamePiece.height/2) && (myGamePiece.speedY!=0) && myGamePiece.y < ((myGameArea.canvas.height / 2) + myGamePiece.height/2) ) &&
       myGamePiece.crash == false)
  {
    if(myCamera.toggleRight == true) {myCamera.newPos();}
    else
    {
      myGamePiece.rightPos();
      if(myGamePiece.x > ((myGameArea.canvas.width / 2) - myGamePiece.width/2) && (myGamePiece.speedX!=0) && myGamePiece.x < ((myGameArea.canvas.width / 2) + myGamePiece.width/2) ||
           myGamePiece.y > ((myGameArea.canvas.height / 2) - myGamePiece.height/2) && (myGamePiece.speedY!=0) && myGamePiece.y < ((myGameArea.canvas.height / 2) + myGamePiece.height/2) )
           {
             myCamera.toggleRight = true;
           }
    }
    if(myCamera.toggleLeft == true) {myCamera.newPos();}
    else
    {
      myGamePiece.leftPos();
      if(myGamePiece.x > ((myGameArea.canvas.width / 2) - myGamePiece.width/2) && (myGamePiece.speedX!=0) && myGamePiece.x < ((myGameArea.canvas.width / 2) + myGamePiece.width/2) ||
           myGamePiece.y > ((myGameArea.canvas.height / 2) - myGamePiece.height/2) && (myGamePiece.speedY!=0) && myGamePiece.y < ((myGameArea.canvas.height / 2) + myGamePiece.height/2) )
           {
             myCamera.toggleLeft = true;
           }
    }
    if(myCamera.toggleUp == true) {myCamera.newPos();}
    else
    {
      myGamePiece.upPos();
      if(myGamePiece.x > ((myGameArea.canvas.width / 2) - myGamePiece.width/2) && (myGamePiece.speedX!=0) && myGamePiece.x < ((myGameArea.canvas.width / 2) + myGamePiece.width/2) ||
           myGamePiece.y > ((myGameArea.canvas.height / 2) - myGamePiece.height/2) && (myGamePiece.speedY!=0) && myGamePiece.y < ((myGameArea.canvas.height / 2) + myGamePiece.height/2) )
           {
             myCamera.toggleUp = true;
           }
    }
    if(myCamera.toggleDown == true) {myCamera.newPos();}
    else
    {
      myGamePiece.downPos();
      if(myGamePiece.x > ((myGameArea.canvas.width / 2) - myGamePiece.width/2) && (myGamePiece.speedX!=0) && myGamePiece.x < ((myGameArea.canvas.width / 2) + myGamePiece.width/2) ||
           myGamePiece.y > ((myGameArea.canvas.height / 2) - myGamePiece.height/2) && (myGamePiece.speedY!=0) && myGamePiece.y < ((myGameArea.canvas.height / 2) + myGamePiece.height/2) )
           {
             myCamera.toggleDown = true;
           }
    }
  }
  else {myGamePiece.newPos();}

  // scrolling camera movement
  if(myGamePiece.x > myCamera.width) {
    prevCam = myCamera.x;
    myCamera.x += myCamera.width;
    if(myCamera.x + myCamera.width > mapW * tileW) {myCamera.x = (mapW * tileW) - myCamera.width;}
    myGamePiece.x -= Math.abs(prevCam - myCamera.x);
  }
  if(myGamePiece.x + myGamePiece.width < 0) {
    prevCam = myCamera.x;
    myCamera.x -= myCamera.width;
    if(myCamera.x < 0) {myCamera.x = 0;}
    myGamePiece.x += Math.abs(prevCam - myCamera.x);
  }
  if(myGamePiece.y > myCamera.height) {
    prevCam = myCamera.y;
    myCamera.y += myGameArea.canvas.height;
    if(myCamera.y + myCamera.height > mapH * tileH) {myCamera.y = (mapH * tileH) - myCamera.height;}
    myGamePiece.y -= Math.abs(prevCam - myCamera.y);
  }
  if(myGamePiece.y + myGamePiece.height < 0) {
    prevCam = myCamera.y;
    myCamera.y -= myCamera.height;
    if(myCamera.y < 0) {myCamera.y = 0;}
    myGamePiece.y += Math.abs(prevCam - myCamera.y);
  }
